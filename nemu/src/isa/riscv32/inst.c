...
// s 是一个指向 Decode 结构体的指针
  ...
  INSTPAT("??????? ????? ????? 111 ????? 00100 11", xori    , I, R(rd) = src1 ^ imm); // Xor Immediate（按位异或立即数），目标寄存器=源寄存器^立即数
  ...
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu     , I, R(rd) = Mr(src1 + imm, 1)); // Load Byte Unsigned（无符号字节加载），把内存中的值加载到目标寄存器中，不进行符号扩展
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu     , I, R(rd) = Mr(src1 + imm, 2)); // Load Halfword Unsigned（无符号半字加载），把内存中的值加载到目标寄存器中，不进行符号扩展
  INSTPAT("??????? ????? ????? 111 ????? 00000 11", andi    , I, R(rd) = src1 & imm); // And Immediate（按位与立即数），目标寄存器=源寄存器&立即数
